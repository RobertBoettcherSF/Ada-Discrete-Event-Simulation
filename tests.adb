...
procedure Tests is
   ...
   overriding procedure Execute (This : in out Dummy_Event; Sim : in out Simulator) is
   begin
      Log_Trace (This.Val);
   end Execute;
   ...
   overriding procedure Execute (This : in out Spawner_Event; Sim : in out Simulator) is
      Child : Event_Access := new Dummy_Event'(Time      => This.Child_Time, 
                                               Id        => 0, 
                                               Cancelled => False, 
                                               Val       => This.Child_Val);
...
begin
   ...
   Reset_Trace;
   Sim.Schedule (new Dummy_Event'(Time => 10.0, Id => 0, Cancelled => False, Val => 42), Dummy_Id);
   ...
   Sim.Schedule (new Dummy_Event'(Time => 20.0, Id => 0, Cancelled => False, Val => 2), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 10.0, Id => 0, Cancelled => False, Val => 1), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 30.0, Id => 0, Cancelled => False, Val => 3), Dummy_Id);
   ...
   Sim.Clear; Reset_Trace;
   Sim.Schedule (new Dummy_Event'(Time => 5.0, Id => 0, Cancelled => False, Val => 1), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 15.0, Id => 0, Cancelled => False, Val => 2), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 25.0, Id => 0, Cancelled => False, Val => 3), Dummy_Id);
   Sim.Run_Until (15.0);
   ...
   Sim.Clear; Reset_Trace;
   Sim.Schedule (new Dummy_Event'(Time => 2.0, Id => 0, Cancelled => False, Val => 1), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 4.0, Id => 0, Cancelled => False, Val => 2), Dummy_Id);
   ...
   begin
      for I in 1 .. 10 loop
         Sim.Schedule (new Dummy_Event'(Time => 10.0, Id => 0, Cancelled => False, Val => Trace_Value_Type(I)), Dummy_Id);
      end loop;
      Check ("9.1 Hit max capacity successfully", Sim.Pending_Events = 10);
      
      -- Attempt 11th
      Sim.Schedule (new Dummy_Event'(Time => 10.0, Id => 0, Cancelled => False, Val => 99), Dummy_Id);
   exception
      when Event_Error => Caught := True;
   end;
   ...
   Sim.Clear; Reset_Trace;
   Sim.Schedule (new Spawner_Event'(Time => 5.0, Id => 0, Cancelled => False, Child_Time => 15.0, Child_Val => 77), Dummy_Id);
   Sim.Step_Next_Event;
   ...
   Sim.Clear;
   Sim.Schedule (new Dummy_Event'(Time => 20.0, Id => 0, Cancelled => False, Val => 1), Dummy_Id);
   ...
   Sim.Clear; Reset_Trace;
   Sim.Schedule (new Dummy_Event'(Time => 5.0, Id => 0, Cancelled => False, Val => 10), Dummy_Id);
   Sim.Schedule (new Dummy_Event'(Time => 5.0, Id => 0, Cancelled => False, Val => 20), Dummy_Id);
   Sim.Run_Until (5.0);
   ...
   Put_Line ("TEST 14 — Negative or Zero Fixed Increment");
   Sim.Clear;
   declare
      Caught_Zero : Boolean := False;
      Caught_Neg  : Boolean := False;
   begin
      begin
      ...
      begin
         Sim.Step_Fixed_Increment (-1.0);
      exception
         when Event_Error => Caught_Neg := True;
      end;
      
      Check ("14.1 Zero increment caught", Caught_Zero);
      Check ("14.2 Negative increment caught", Caught_Neg);
      Check ("14.3 Clock safely unchanged", Sim.Current_Time = 0.0);
   end;
